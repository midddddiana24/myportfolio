...
function TechCategorySection({ category, idx }: { category: TechCategory; idx: number }) {
  ...
  return (
    <section className="py-16 border-b" style={{ borderColor:'var(--border)', background: idx % 2 === 0 ? 'var(--bg-base)' : 'var(--bg-surface)' }}>
      <div className="rm-container">
        <div className="grid grid-cols-1 lg:grid-cols-[200px_1fr] gap-10 items-start">
          {/* Label */}
          <ClipReveal direction="right">
            <div>
              <p style={{ fontFamily:"'DM Mono', monospace", fontSize:'0.6875rem', letterSpacing:'0.15em', textTransform:'uppercase', color:'var(--accent)', marginBottom:'0.5rem' }}>
                {String(idx + 1).padStart(2, '0')}
              </p>
              <h2 style={{ fontFamily:"'Anton', sans-serif", textTransform:'uppercase', fontWeight:400, fontSize:'1.25rem', letterSpacing:'0em', color:'var(--text-1)', marginBottom:'0.5rem' }}>
                {info.label}
              </h2>
              <p style={{ fontFamily:"'DM Mono', monospace", fontSize:'0.5625rem', letterSpacing:'0.08em', color:'var(--text-muted)' }}>
                {info.description}
              </p>
            </div>
          </ClipReveal>
          ...
          {/* Icon grid */}
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-px" style={{ background:'var(--chip)' }}>
            {items.map((tech, i) => {
              ...
              return (
                <motion.div key={tech.name}
                  ...
                  viewport={{ once: true }}
                  style={{ background: idx % 2 === 0 ? 'var(--bg-base)' : 'var(--bg-surface)', padding:'1.5rem 1rem', display:'flex', flexDirection:'column', alignItems:'center', gap:'0.625rem', transition:'background 0.2s' }}
                  onMouseEnter={e => { e.currentTarget.style.background='#1a1a1a' }}
                  onMouseLeave={e => { e.currentTarget.style.background = idx % 2 === 0 ? 'var(--bg-base)' : 'var(--bg-surface)' }}
                >
                  <Icon size={26} className="tech-icon" style={{ flexShrink:0 }} />
                  <span style={{ fontFamily:"'DM Mono', monospace", fontSize:'0.5625rem', letterSpacing:'0.08em', textTransform:'uppercase', color:'var(--text-muted)', textAlign:'center', lineHeight:1.3 }}>
                    {tech.name}
                  </span>
                </motion.div>
              )
...
export default function TechStack() {
  ...
  return (
    <PageTransition className="pt-28">
      ...
      {/* Marquee strip */}
      <div className="py-8 border-y overflow-hidden" style={{ borderColor:'var(--border)', background:'var(--bg-surface)' }}>
        <Marquee speed="normal" gap="3rem">
          {allIcons.map(({ name, Icon }) => (
            <div key={name} className="flex items-center gap-2.5 flex-shrink-0">
              ...
              <span style={{ fontFamily:"'DM Mono', monospace", fontSize:'0.6875rem', letterSpacing:'0.1em', textTransform:'uppercase', color:'var(--text-subtle)', whiteSpace:'nowrap' }}>
                {name}
              </span>
            </div>
          ))}
      ...
      <section className="py-16" style={{ background:'var(--bg-base)' }}>
        <div className="rm-container">
          <ClipReveal direction="down">
            <div style={{ border:'1px solid var(--border)', padding:'2rem', display:'flex', alignItems:'flex-start', gap:'1.5rem' }}>
              <span style={{ fontFamily:"'DM Mono', monospace", fontSize:'0.6875rem', letterSpacing:'0.1em', color:'var(--accent)', flexShrink:0 }}>NOTE</span>
