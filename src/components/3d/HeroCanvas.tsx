      ...
      {bursts.map(burst =>
        burst.particles.filter(p => p.life > 0).map((p, i) => (
          <mesh key={`${burst.id}-${i}`} position={p.pos}>
            ...
            <meshBasicMaterial color="#e4e4e4" transparent opacity={Math.max(0, p.life)} />
          </mesh>
        ))
      )}
    </>
    ...
    <Float speed={1.6} rotationIntensity={0.4} floatIntensity={0.8}>
      <group onClick={handleClick}>
        <mesh ref={mesh}>
          ...
          <MeshDistortMaterial color="#ffffff" distort={0.25} speed={2.5}
...
function ParticleField({ count = 2500, scrollProgress }: { count?: number; scrollProgress: number }) {
  ...
  const [positions, colors] = useMemo(() => {
    ...
    const c2 = new THREE.Color('#c8c8c8')
...
function EnergyRing({ radius, speed, tilt, scrollProgress }: {
  ...
  return (
    <mesh ref={ring}>
      ...
      <meshBasicMaterial color="#e4e4e4" transparent opacity={0.3} />
    </mesh>
  )
}
...
function SceneLights() {
  ...
  return (
    <>
      ...
      <pointLight ref={l1} color="#ffffff" intensity={4} position={[5, 3, 5]}  distance={15} />
      <pointLight ref={l2} color="#c8c8c8" intensity={3} position={[-4,-2, 3]} distance={12} />
    ...
    <div className="w-full h-full" onMouseMove={handleMouseMove} aria-hidden="true">
      ...
      >
        <Suspense fallback={null}>
          ...
          <OrbitingShape radius={3.2} speed={0.4}  offset={0}           color="#c8c8c8" size={0.18} scrollProgress={scrollProgress} />
          <OrbitingShape radius={3.8} speed={0.3}  offset={Math.PI}     color="#ffffff" size={0.14} scrollProgress={scrollProgress} />
          <OrbitingShape radius={2.9} speed={0.55} offset={Math.PI/2}   color="#ffffff" size={0.10} scrollProgress={scrollProgress} />
          <OrbitingShape radius={4.2} speed={0.25} offset={Math.PI*1.5} color="#ffffff" size={0.12} scrollProgress={scrollProgress} />
